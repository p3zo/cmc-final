...
from fast_transformers.builders import (
    RecurrentEncoderBuilder as RecurrentEncoderBuilder_local,
)
from fast_transformers.builders import (
    TransformerEncoderBuilder as TransformerEncoderBuilder_local,
)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, x, target, loss_mask):
        ...
        if len(self.n_token) == 9:
            loss_key = self.compute_loss(y_key, target[..., 8], loss_mask)
    ...
    def froward_output_sampling(self, h, y_type, is_training=False):
        ...
        if is_training:
            ...
            type_word_t = (
                torch.from_numpy(np.array([cur_word_type])).long().cuda().unsqueeze(0)
            )  # shape = (1,1)

        tf_skip_type = self.word_emb_type(type_word_t).squeeze(
            0
        )  # shape = (1, embd_size)
    ...
    def inference_from_scratch(
        self, dictionary, emotion_tag, key_tag=None, n_token=8, display=True
    ):
        ...
        if n_token == 9:
            ...
            if key_tag:
                ...
                target_emotion = [0, 0, 0, 1, 0, 0, 0, emotion_tag, 0]
                init = np.array(
                    [target_emotion, [0, 0, 1, 2, 0, 0, 0, 0, 0]]  # emotion  # bar
                )
            ...
            init = np.array(
                [target_emotion, [0, 0, 1, 2, 0, 0, 0, 0]]  # emotion  # bar
            )
