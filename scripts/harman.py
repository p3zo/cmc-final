...
NOTE: This script depends on a private dependency: `chord_progressions`. This
is a package that I developed but have not currently open-sourced. This script
won't run for you until I publish it to a public package index or include it as
...
def write_harman_labels(inpath, outpath, arrangement_id):
    try:
        labels = label_file(inpath)
        ...
        logger.debug(f"Failed to label {inpath}: {e}")
        return {None: inpath}

    if not labels:
        logger.debug(f"Failed for {inpath}")
        return {None: inpath}
